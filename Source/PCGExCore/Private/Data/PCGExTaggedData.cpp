...
FPCGExTaggedData::FPCGExTaggedData(const TSharedPtr<PCGExData::FPointIO>& InData, const int32 InIdx)
	: Data(InData->GetIn())
	  , Index(InIdx == INDEX_NONE ? InData->IOIndex : InIdx)
	  , Tags(InData->Tags)
	  , Keys(InData->GetInKeys())
{
}

TSharedPtr<PCGExData::FTags> FPCGExTaggedData::GetTags() const
{
	return Tags.Pin();
}
...
{
	InOut.Data = Data;
	if (const TSharedPtr<PCGExData::FTags> PinnedTags = Tags.Pin();
		PinnedTags.IsValid())
	{
		PinnedTags->DumpTo(InOut.Tags);
	}
};
