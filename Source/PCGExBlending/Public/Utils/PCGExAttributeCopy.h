		...
		const TSharedPtr<PCGExData::TBuffer<T>>& OutBuffer)
	{
		...
		if (OutBuffer->GetUnderlyingDomain() == PCGExData::EDomainType::Elements)
		{
			...
			if (bSourceIsData)
			{
				...
				// 5.7 has no base-ptr ReadDataValue overload; InAttribute is a FPCGMetadataAttribute<T> here, so cast.
				const T Value = PCGExData::Helpers::ReadDataValue(static_cast<const FPCGMetadataAttribute<T>*>(InAttribute));
		...
		else if (OutBuffer->GetUnderlyingDomain() == PCGExData::EDomainType::Data)
		{
			...
			if (bSourceIsData)
			{
				OutDataBuffer->SetValue(0, PCGExData::Helpers::ReadDataValue(static_cast<const FPCGMetadataAttribute<T>*>(InAttribute)));
			}
			else
			{
				TUniquePtr<const IPCGAttributeAccessor> InAccessor = PCGAttributeAccessorHelpers::CreateConstAccessor(InAttribute, InAttribute->GetMetadataDomain());
